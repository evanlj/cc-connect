...
func buildSquadPlannerPrompt(run *SquadRun) string {
	...
	b.WriteString("输出要求（必须同时满足）：\n")
	b.WriteString("A) 先输出“思路摘要”：说明你如何拆分任务、关键依赖与风险（3~8条）；\n")
	b.WriteString("   - 只写自然语言，不要在 A 段使用 JSON 花括号 `{}`；\n")
	b.WriteString("B) 再输出“JSON结果”：只输出 1 个 JSON 对象，并放在 ```json 代码块``` 中。\n\n")
	b.WriteString("B 段 JSON 格式：\n")
	b.WriteString("{\n")
	b.WriteString("  \"title\": \"\",\n")
...
func buildSquadExecutorPrompt(run *SquadRun, task SquadTask, round int) string {
	...
	b.WriteString("输出要求：\n")
	b.WriteString("A) 输出“思路摘要”：简述本轮实施策略、关键修改点、验证结果（3~6条）；\n")
	b.WriteString("   - A 段只写自然语言，不要使用 JSON 花括号 `{}`；\n")
	b.WriteString("B) 输出“JSON结果”：只输出 1 个 JSON 对象，并放在 ```json 代码块``` 中。\n")
	b.WriteString("B 段 JSON 格式：\n")
...
func buildSquadReviewerPrompt(run *SquadRun, task SquadTask, executorReply string, changedFiles []string, testResult string) string {
	...
	b.WriteString("输出要求（必须同时满足）：\n")
	b.WriteString("A) 先输出“思路摘要”：仅说明你的审核依据、命中的验收点、发现的问题类型（3~6条）；\n")
	b.WriteString("   - A 段只写自然语言，不要使用 JSON 花括号 `{}`；\n")
	b.WriteString("B) 再输出“JSON结果”：只输出 1 个 JSON 对象，并放在 ```json 代码块``` 中。\n")
	b.WriteString("B 段 JSON 格式：\n")
