...
set "CFG=%ROOT%\instances\instance-c-translator\config.toml"
...
echo [INFO] Starting instance-c-translator (trace translate only)
...
set "RUN_DIR=%ROOT%\instances\instance-c-translator\data\run"
...
if "%PID%"=="" (
  echo [ERROR] failed to start instance-c-translator
  exit /b 1
)

echo %PID%>"%PID_FILE%"
echo [OK] started instance-c-translator PID=%PID% => "%EXE%" -config "%CFG%"
echo [LOG] %ROOT%\instances\instance-c-translator\data\run\cc-connect.log
exit /b 0
