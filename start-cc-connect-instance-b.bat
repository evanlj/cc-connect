...
set "CFG=%ROOT%\instances\instance-b\config.toml"
...
set "RUN_DIR=%ROOT%\instances\instance-b\data\run"
...
if "%PID%"=="" (
  echo [ERROR] failed to start instance-b
  exit /b 1
)

echo %PID%>"%PID_FILE%"
echo [OK] started instance-b PID=%PID% => "%EXE%" -config "%CFG%"
echo [LOG] %ROOT%\instances\instance-b\data\run\cc-connect.log
exit /b 0
