...
set "CFG=%ROOT%\instances\instance-a\config.toml"
...
set "RUN_DIR=%ROOT%\instances\instance-a\data\run"
...
if "%PID%"=="" (
  echo [ERROR] failed to start instance-a
  exit /b 1
)

echo %PID%>"%PID_FILE%"
echo [OK] started instance-a PID=%PID% => "%EXE%" -config "%CFG%"
echo [LOG] %ROOT%\instances\instance-a\data\run\cc-connect.log
exit /b 0
