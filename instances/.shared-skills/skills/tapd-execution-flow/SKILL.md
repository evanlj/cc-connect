...
## Anti-Detour Defaults (Fast Lane)
Use these defaults to avoid “thinking loops” and reduce time-to-result.  
If a situation requires exceptions, ask the user **once** and proceed only after they confirm.

### 1) Results are binary: PASS / FAIL only
- Overall acceptance result must be **PASS** or **FAIL** (no BLOCK / PARTIAL / UNKNOWN).
- If any required acceptance item is **not executed** or **missing evidence**, treat it as **FAIL** and set overall result to **FAIL**.
  - You may explain the reason in the acceptance process/evidence (e.g. “blocked by env / pending CI smoke”), but keep the result field PASS/FAIL.
- Status suggestion must follow the binary result:
  - PASS -> `status_6`
  - FAIL -> keep `status_4`

### 2) One-shot questions at gates
- At each mandatory gate, ask at most **one** clarifying question.
- Provide **2–3 concrete options** (with a recommended one) instead of open-ended discussion.
- Do **not** output long meta-reasoning to the user; keep user-facing messages concise (summary + decision request).

### 3) Stay on the critical path
- Do **not** do “cleanup / formatting perfection / optional refactors” unless it is required by the DoD or blocks execution.
- Do **not** delete files or perform other destructive actions unless the user explicitly asked (or it is a required remediation step).

### 4) Budget limits (default)
- Repo exploration: max **3** searches before you must propose a plan with current info.
- Log digging: max **3** keyword searches + top error summary before proposing next steps.
- Evidence: collect only the **minimum** artifacts required by DoD; avoid “nice-to-have” extras.
...
If user did not specify planning model:
- Ask **one** question and provide options, for example:
  - Planning model: `o4-mini` (recommended) / `o3` / “same as main model”
- Do not proceed to Step 3 until the user explicitly selects one.
...
### Planner prompt (dynamic generation rule)
When the planning content is generated by a model (instead of manually drafted), treat it as a dedicated **Planner** role prompt.
- Build it with `references/prompt-spec.md`:
  - apply **Global Fast Lane Block** (section 0)
  - follow **Planner Prompt Contract** (section 1)
- Planner output must stop at: baseline plan + DoD + risk + one-shot decision question (if any).
- Do not jump into code execution in this step.
...
- For dynamic prompt generation, use role contracts in `references/prompt-spec.md`:
  - section 2: **Execution Prompt Contract**
  - section 3: **Acceptance Prompt Contract**
  - section 4: **Prompt Quality Gate** (must pass before asking user confirmation)
- Always prepend the **Global Fast Lane Block** (section 0) to execution/acceptance prompts.
...
- If an item is “not executed / pending CI / blocked by environment”, still mark the item result as **FAIL** and explain the reason in process/evidence.
...
- `references/prompt-spec.md`: role-based dynamic prompt contracts (Planner / Executor / Acceptor).
